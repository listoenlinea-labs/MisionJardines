(() => {
  const initNavigationDrawer = () => {
    ...
    if (!nav || !menu || menu.dataset.drawerReady === 'true') {
      return;
    }
    ...
    menu.id = menu.id || 'mainNavigationDrawer';
    ...
    style.textContent = `
      ...
      .nav-drawer-toggle-icon::before,
      .nav-drawer-toggle-icon::after {
        width: 22px;
        height: 2px;
        display: block;
        border-radius: 999px;
      ...
      .nav-drawer-toggle-icon::before { top: -7px; }
      .nav-drawer-toggle-icon::after { top: 7px; }

      .nav-drawer-toggle.is-open {
        position: fixed;
        top: 20px;
        left: 20px;
      }

      .nav-drawer-toggle.is-open .nav-drawer-toggle-icon {
        background: transparent;
      }

      .nav-drawer-toggle.is-open .nav-drawer-toggle-icon::before {
        top: 0;
        transform: rotate(45deg);
      }

      .nav-drawer-toggle.is-open .nav-drawer-toggle-icon::after {
        top: 0;
        transform: rotate(-45deg);
      }
      ...
      .nav-drawer {
        ...
        background:
          radial-gradient(circle at 15% 8%, rgba(94,234,212,.13), transparent 28%),
          linear-gradient(180deg, rgba(7,17,31,.995), rgba(10,23,40,.995));
      ...
      .nav-drawer a {
        ...
        color: var(--muted, #aebbd0) !important;
        background: transparent;
        text-align: left;
      ...
      .nav-drawer a.active {
        color: #fff !important;
        background: linear-gradient(90deg, rgba(94,234,212,.16), rgba(96,165,250,.12)) !important;
    ...
    const setOpen = (open) => {
      ...
      } else if (lastFocusedElement instanceof HTMLElement && lastFocusedElement !== toggle) {
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initNavigationDrawer, { once: true });
  } else {
